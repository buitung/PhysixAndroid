...
public class VectorDotProductActivity extends Activity {
	...
	public void calculate(View view) throws ParsingException {
		...
		try {
			try {
				EditText editText = (EditText) findViewById(R.id.txtVectorXVectorDotProduct1);
				x1 = Float.parseFloat(editText.getText().toString());
				editText = (EditText) findViewById(R.id.txtVectorYVectorDotProduct1);
				y1 = Float.parseFloat(editText.getText().toString());
				editText = (EditText) findViewById(R.id.txtVectorXVectorDotProduct2);
				x2 = Float.parseFloat(editText.getText().toString());
				editText = (EditText) findViewById(R.id.txtVectorYVectorDotProduct2);
				y2 = Float.parseFloat(editText.getText().toString());
			} catch (Exception e) {
				throw new ParsingException(
						"Please enter inputs in the right form");
			}
			EditText editText = (EditText) findViewById(R.id.txtVectorZVectorDotProduct1);
			...
			editText = (EditText) findViewById(R.id.txtVectorZVectorDotProduct2);
			if (editText.getText().toString().trim().length() == 0)
				z2 = 0;
			else {
				try {
			...
			TextView txtView = (TextView) findViewById(R.id.lblDotProductVectorDotProduct);
			txtView.setText(RoundingRounding.RRR(Float.toString(vector1.DotProduct(vector2))));
